...
public:
	...
	void Add(TutorialPlayer* object);
	void Add(TutorialBottom* object);
	void Add(Tile_dir* object);
	...
	void Remove(TutorialPlayer* object);
	void Remove(TutorialBottom* object);
	void Remove(Tile_dir* object);
	...
	const vector<TutorialPlayer*>& GetTutorailPlayer() { return _tutorial_player; } //외부 CPP,헤더파일에서 벡터가져오기//
	const vector<TutorialBottom*>& GetTutorailBottom() { return _tutorial_bottom; } //외부 CPP,헤더파일에서 벡터가져오기//
	const vector<Tile_dir*>& GetTileDir() { return _vtileDir; } //외부 CPP,헤더파일에서 벡터가져오기//
...
private:
	...
	vector<Tile_dir*> _vtileDir;


	vector<TutorialPlayer*> _tutorial_player;
	vector<TutorialBottom*> _tutorial_bottom;
